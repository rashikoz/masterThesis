...
####################################################################################################
# Now analysis  
testFolder = rusinol.testPath
resultsFolder = rusinol.resultPath
gTruthFolder = os.path.join(rusinol.testPath, 'groundTruth')
...
###################################################################################################
# calculate max image
maxPredictFileList = os.listdir(predictFolder)
numMaxFiles = len(maxPredictFileList) 
print 'Reading folder - ', predictFolder
...
for eachMaxPredictFile in maxPredictFileList:
    if eachMaxPredictFile.endswith(".npy"):
        if eachMaxPredictFile.startswith('MaxPredict_'):
            maxPredict = np.load(os.path.join(predictFolder, eachMaxPredictFile))
...
if(checkPresence):
    ...
    for npyFile in maxPredictFileList:
        if npyFile.endswith(".npy"):        
            if npyFile.startswith('MaxPredict_'):
                print imageRun, '. Loading - ',npyFile
                # get the file name
                fName = npyFile.partition('MaxPredict_')[2]
                ...
                #gTruthFilename = gTruthFilename.replace('.pkl', '.npy')
...
plt.clf()
plt.plot(finalRecall, finalPrecision, label= 'SIFT - ' + \
         ' AUC={0:0.3f}'.format(auc) + \
...
print 'Training Path - ', rusinol.trainPath
print 'Testing Path - ', rusinol.testPath
